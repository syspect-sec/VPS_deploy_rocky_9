...
do
  if [ ${user[0]} != "root" ]; then
    ...
    mv /root/.ssh/authorized_keys /home/${user[0]}/.ssh/authorized_keys
...
#
# Install PHP 7 and necessary extensions
#
echo "[Installing PHP...]"
...
then
  ...
  do
    if [ ${phpversion} = "8.2" ]; then
      ...
      dnf install -y php php-common php-gd php-mysqlnd php-curl php-mcrypt php-json
      rpm -qa | grep php
    if [ ${phpversion} = "8.1" ]; then
      ...
      dnf install dnf-utils  http://rpms.remirepo.net/enterprise/remi-release-7.rpm
      yum update -y
      dnf update --refresh
      yum install -y yum-utils
      dnf module reset php
...
then
  ...
  do
    if [ ${mysqlpass[0]} = "root" ]; then
      ...
      #TODO: update for Centos 9 and Mariadb 10
      ...
      mysql -e "GRANT GRANT SELECT, INSERT, UPDATE, DELETE ON <default_github_reponame>.* TO '${mysqlpass[0]}'@'localhost';"
...
then
    ...
    do
      ...
      if [[ ${githubuser[0]:0:1} != "#" && ! -z "${githubuser[0]}" ]]; then
        ...
        #TODO: permissions on sites available and sites enabled
...
# Install Redis cache
echo "[Installing Redis cache packages...]"
yum install -y redis
# Restart Apache
